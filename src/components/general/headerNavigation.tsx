...
export default function HeaderNavigation() {
    ...
    return (
        <div id="navbar" className="w-full fixed flex flex-row justify-between duration-300 items-center bg-tera-green px-10 nav">
                        ...
                        <HoverCard className="nav-links" key={`${item.navigationName}-${index}`} openDelay={100} closeDelay={0}>
                            <div className="group w-full text-center">
                                <HoverCardTrigger asChild>
                                    <Link href={item.link ? item.link :"/"} className="bg-tera-green text-white w-full group-hover:bg-green-500 px-4 py-2 group-hover:text-black">{item.navigationName} </Link>
                                </HoverCardTrigger>
